...
const Renderer = {
    ...
    drawSlime(ctx, x, y, w, h, color, darkColor, dir = 1, frame = 0, vy = 0, slimeType = 'slime') {
        ...
        if (isEnemy && enemyTheme === 'mecha' && !showInterior) {
            const pulse = 0.55 + Math.sin(_getFrameNow() * 0.01) * 0.2;
            ctx.save();
            ctx.strokeStyle = `rgba(94, 211, 255, ${0.45 + pulse * 0.25})`;
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.arc(cx, dCY - dRY * 0.1, 72, Math.PI * 0.1, Math.PI * 0.9);
            ctx.stroke();

            for (let i = -2; i <= 2; i++) {
                const px = cx + i * 26;
                const py = dCY + dRY * 0.15;
                Renderer._roundRect(ctx, px - 10, py - 7, 20, 14, 4);
                ctx.fillStyle = `rgba(30, 45, 55, 0.78)`;
                ctx.fill();
            ...
            [-1, 1].forEach(side => {
                const baseX = cx + side * (bw * 0.34);
                const baseY = dCY - dRY * 0.1;
                ctx.fillStyle = '#32414C';
                ctx.beginPath();
                ctx.moveTo(baseX, baseY - 18);
                ctx.lineTo(baseX + side * 30, baseY - 4);
                ctx.lineTo(baseX + side * 30, baseY + 16);
                ...
                ctx.fillStyle = `rgba(94, 211, 255, ${0.35 + pulse * 0.2})`;
                ctx.beginPath();
                ctx.arc(baseX + side * 18, baseY + 5, 5, 0, Math.PI * 2);
                ctx.fill();
            });
            ctx.restore();
        }

        if (isEnemy && enemyTheme === 'heaven' && !showInterior) {
            const pulse = 0.45 + Math.sin(_getFrameNow() * 0.008) * 0.18;
            ctx.save();
            ctx.strokeStyle = `rgba(241, 201, 107, ${0.55 + pulse * 0.2})`;
            ctx.lineWidth = 4;
            ctx.beginPath();
            ctx.arc(cx, dCY - dRY - 26, 30, 0, Math.PI * 2);
            ctx.stroke();

            ctx.strokeStyle = 'rgba(255,255,255,0.65)';
            ctx.lineWidth = 2;
            [-1, 1].forEach(side => {
                const wx = cx + side * (bw * 0.43);
                const wy = dCY - dRY * 0.12;
                ctx.beginPath();
                ctx.moveTo(wx, wy);
                ctx.quadraticCurveTo(wx + side * 34, wy - 18, wx + side * 52, wy + 16);
                ctx.quadraticCurveTo(wx + side * 32, wy + 10, wx + side * 16, wy + 24);
                ctx.stroke();
                ctx.beginPath();
                ctx.moveTo(wx, wy + 8);
                ctx.quadraticCurveTo(wx + side * 26, wy + 2, wx + side * 42, wy + 28);
                ctx.stroke();
            });

            for (let i = -1; i <= 1; i++) {
                const gemX = cx + i * 28;
                const gemY = dCY + dRY * 0.05;
                ...
                ctx.closePath();
                ctx.fill();
                ctx.strokeStyle = 'rgba(241, 201, 107, 0.75)';
                ctx.lineWidth = 1;
                ctx.stroke();
            }
            ctx.restore();
