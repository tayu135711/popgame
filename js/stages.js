...
const STAGES_CHAPTER5 = [
    ...
    // ============================================================
    // CH5 BOSS - 👑 真の王・スライム王（最終決戦・三形態大演出）
    // ============================================================
    {
        id: 'c5_boss',
        isChapter5: true,
        isBoss: true,
        isFinalBoss: true,
        isSlimeKingBoss: true,        // ★スライム王専用ギミック全有効化（三形態・復活・潜り込み・コア奪取）
        hasPhaseTwo: false,            // 形態変化はbattle.js側で独自管理
        hasEntranceAnim: true,         // ★豪華入場演出フラグ
        enemyTankTheme: 'slime_king',
        name: '👑 真の王・スライム王——最終決戦',
        desc: '全ての旅の果て——世界の始まりの頂で、伝説のコアを宿した王が真の力を解放する。三段階の形態変化、復活ギミック、潜り込み攻撃、そしてコア奪取変身。これが本当に最後の戦いだ！！',
        enemyHP: 90000,
        playerHP: 750,
        enemyFireInterval: 36,
        enemyDamage: 210,
        ...
        // 夜明けの黄金天空：ドラマチックな夕日→闇→金の空
        skyColors: ['#0a0500', '#1f0900', '#3a1200', '#0a0500'],
        reward: [
            'legendary_core', 'legendary_core', 'legendary_core', 'legendary_core',
            'legendary_core', 'ultimate_parts', 'ultimate_parts', 'master_emblem',
        ],
        invasion: { switches: 10, defenders: 10, lasers: 10 },
        partReward: [
            { id: 'skin_dragon',     category: 'skins', name: '🐉 黄金神龍タンク',  icon: '🐉' },
        ...
        // ──────────────────────────────────────────────────────────
        // 入場演出セリフ（画面暗転＋スライム王戦車が滑り込む演出中）
        // ──────────────────────────────────────────────────────────
        entranceLines: [
            '……よく来たな、スラりん。',
            'この世界の始まりの場所まで——たどり着くとは。',
            '村のあの小さなスライムが、ここまで。',
            '……正直、驚いているぞ。わしが。',
            'だが——だからこそ、手は抜かん。',
            '王の全力を、受けてみせろ。',
            'これが最後の戦いだ——全力で——かかって来い！！！',
        ],
        // ──────────────────────────────────────────────────────────
        // 戦闘前ダイアログ（入場演出後に表示）
        // ──────────────────────────────────────────────────────────
        dialogue: [
            { speaker: 'スラッチ', text: '……スライム王。こんなに遠くまで、来てたんですね。ずっと、探してました。' },
            { speaker: 'ベス', text: 'あたしも！あなたを見つけるためだけに、第5章まで来たんだから！' },
            { speaker: 'ニヒルム', text: '……あなたが、伝説のコアを持つ者か。この気配——私が長い時間、恐れていたものと同じだ。' },
            { speaker: 'スライム王', text: 'ニヒルム——お前も来たか。闇の王が、今は誰かの隣に立っている。……それだけで、もう十分だ。' },
            { speaker: 'スラりん', text: 'スライム王……！ずっと探してた。一緒に、みんなのところへ帰ろう！' },
            { speaker: 'スライム王', text: '……フフ。「帰る」か。わしを見つけるためだけに、原初の空の果てまで来たか、スラりん。' },
            { speaker: 'スライム王', text: 'だが——わしは王だ。優しい言葉に、ここは動かん。お前の「本気」を見せてみろ。' },
            { speaker: 'スライム王', text: 'このでんせつのコアに宿る全ての力——今、お前にぶつけてやる。受け止めてみせろ、スラりん！！！' },
            { speaker: 'スラりん', text: '……わかった。全部受け止める。倒して——一緒に帰る。それが、ぼくたちの答えだ！！！' },
        ],
        // ──────────────────────────────────────────────────────────
        // 敗北後ダイアログ（コア奪取変身イベント後、エンディング前に表示）
        // ──────────────────────────────────────────────────────────
        defeatDialogue: [
            { speaker: 'スライム王', text: '……ハハ。やるじゃないか。本当に、やるじゃないか——スラりん。' },
            { speaker: 'スライム王', text: 'このコアの輝き——お前が持つ方が、ふさわしい。受け取れ。これはわしからの、最後の贈り物だ。' },
            { speaker: 'スラりん', text: 'スライム王……！ありがとう。でも、これはふたりの力だ。一人じゃ絶対に手に入らなかった。' },
            { speaker: 'ニヒルム', text: '……（静かに）「帰る場所」が、できた。私には長い間、それがなかった。……今は、ある。' },
            { speaker: 'ベス', text: 'スライム王、一緒に帰ってくれますよね！？村のみんな、ずっと待ってるんですよ！' },
            { speaker: 'スラッチ', text: 'あなたのために作ったご飯が、ずっとテーブルの上に置いてあるんです。冷めちゃってるけど——' },
            { speaker: 'スライム王', text: '——バカ者。そんな話を今するか。……フン。仕方ないな。王が直々についていってやろう。光栄に思え！' },
            { speaker: 'スラりん', text: '決まりだ！みんなで帰ろう——ぼくらの村に、全員でっ！！' },
