...
const UI = {
    ...
    drawStageSelect(ctx, W, H, selectedIdx, saveData, frame, difficultyMode = false, selectedDifficulty = 'NORMAL') {
        ...
        const normalStages = STAGES.filter(s => !s.isEvent && !s.isExtra);
    ...
    drawAllyEdit(ctx, W, H, unlocked, deck, cursor, frame = 0) {
        ...
        unlocked.forEach((ally, i) => {
            ...
            if (drawFunc === Renderer.drawSlime) {
                ...
                // Alias function handling (pass all arguments if possible)
                if (rendererType === 'special' || rendererType === 'titan_golem' || rendererType === 'metalking' || rendererType === 'healer' || rendererType === 'ghost' || rendererType === 'ultimate') {
            ...
            ctx.textAlign = 'left';
            ctx.textBaseline = 'middle';
            ctx.fillText(ally.name, leftX - 20, y - 5);

            // === マーク表示 ===
            // ⚗ = 配合産キャラ（配合で生まれた）
            // 🔀 = 配合素材になれるキャラ（今すぐ配合できる）
            ...
            if (ally.isFusion) {
                ...
                const markColor = inDeck ? '#555' : (depth >= 2 ? '#FFB347' : '#7CFC00');
                ctx.font = 'bold 11px Arial';
                ctx.fillStyle = markColor;
                ctx.fillText('⚗配合産', leftX + 60, y - 5);
            } else if (isFusionable) {
                ctx.font = 'bold 11px Arial';
                ctx.fillStyle = inDeck ? '#555' : '#00BCD4';
                ctx.fillText('🔀配合可', leftX + 60, y - 5);
            }
            ...
            ctx.fillText('★'.repeat(allyRarity) + '☆'.repeat(Math.max(0, 6 - allyRarity)), leftX - 20, y + 12);

            // Cost indicator（星の後ろに大型バッジ）
            if (allyCost === 2) {
                ctx.font = '11px Arial';
                ctx.fillStyle = inDeck ? '#555' : '#FFD700';
                ctx.fillText('  大型', leftX - 20 + (allyRarity * 13), y + 12);
            }
    ...
    _drawGachaResult(ctx, W, H, ally) {
        ...
        if (df===Renderer.drawSlime||['special','titan_golem','metalking','healer','ghost','ultimate'].includes(rType)) {
        ...
        if (tab === 0) {
            ...
            masterAllyList.forEach((item, i) => {
                ...
                if (discovered) {
                    ...
                    if (df === Renderer.drawSlime || rType === 'special' || rType === 'titan_golem' || rType === 'metalking' || rType === 'healer' || rType === 'ghost' || rType === 'ultimate') {
    ...
    drawGachaAdventureAnim(ctx, W, H, rarity, timer, frame) {
        ...
        const alpha = timer < 20 ? timer / 20 : Math.min(1, timer > 80 ? (100 - timer) / 20 : 1);

        // レア度ごとのテーマ
        const themes = {
            1: { bg1: '#1B5E20', bg2: '#2E7D32', label: '草原へ！', stars: 1, color: '#81C784', particles: '🌿' },
            2: { bg1: '#1B5E20', bg2: '#388E3C', label: '丘を越えて！', stars: 2, color: '#66BB6A', particles: '🌱' },
            3: { bg1: '#1A3A0A', bg2: '#2E5A1A', label: '深い森の奥へ！', stars: 3, color: '#4CAF50', particles: '🌲' },
            4: { bg1: '#0D2A6E', bg2: '#1565C0', label: '神秘の海底へ！', stars: 4, color: '#42A5F5', particles: '✨' },
            5: { bg1: '#F57F17', bg2: '#FBC02D', label: '天国の頂へ！', stars: 5, color: '#FFD700', particles: '⭐' },
            6: { bg1: '#0A0020', bg2: '#1A004A', label: '宇宙の果てへ！！', stars: 6, color: '#E040FB', particles: '💫' },
        };
        ...
        // 星・光のパーティクル
        const particleCount = 8 + rarity * 2;
        for (let i = 0; i < particleCount; i++) {
            const px = ((frame * (1 + i * 0.3) + i * 100) % (W + 40)) - 20;
            const py = 50 + (i % 5) * 100 + Math.sin(frame * 0.05 + i) * 30;
            ctx.font = '20px Arial';
            ctx.textAlign = 'center';
            ctx.fillText(theme.particles, px, py);
        }

        // スライムキャラが走るアニメーション
        const slimeX = -60 + progress * (W + 120);
        const slimeY = H * 0.55;
        ...
        // 速度に応じた傾き
        ctx.rotate(progress < 1 ? 0.15 : 0);
        if (window.Renderer) {
            window.Renderer.drawSlime(ctx, -30, -30, 60, 60,
                theme.color, theme.bg1, 1, frame, bounce > 5 ? 3 : 0, 'slime');
        }
        // 足跡エフェクト
        ctx.restore();
        ...
        ctx.fillStyle = '#FFF';
        ctx.textAlign = 'center';
        const bounce2 = Math.sin(frame * 0.1) * 3;
        ctx.fillText(theme.label, W / 2, H * 0.25 + bounce2);
        ctx.shadowBlur = 0;

        // 星表示
        ctx.font = 'bold 28px Arial';
        ctx.fillStyle = theme.color;
        const stars = '★'.repeat(theme.stars) + '☆'.repeat(6 - theme.stars);
        ctx.fillText(stars, W / 2, H * 0.25 + 45);

        // 「スライムが旅に出た！」テキスト
        if (timer > 40) {
            ctx.globalAlpha = alpha * Math.min(1, (timer - 40) / 20);
            ctx.font = '20px Arial';
            ctx.fillStyle = 'rgba(255,255,255,0.9)';
            ctx.fillText('スライムが旅に出た！', W / 2, H * 0.75);
        }
