...
class AllySlime {
    ...
    updateAutoSkill(invader, g) {
        ...
        // 防御系: インベーダー出現 or プレイヤーHP 50%以下でバリア
        const TANK_TYPES = ['royal_guard', 'titan_golem', 'platinum_golem'];
            ...
            for (let i = 0; i < 3; i++) {
                this.burstQueue.push({
                    delay: i * 5, fn: () => {
                        ...
                        window.game.projectiles.push(new SimpleProjectile({
                            x: myX, y: myY,
                            vx: dir * 10, vy: (Math.random() - 0.5) * 2,
                            life: 75, damage: this.damage * 1.4 | 0,
                            w: 18, h: 18, type: 'magic', color: '#FF4500'
                        }));
                    }
                });
            }
            g.particles.rateEffect(this.x, this.y - 20, '烈火！', '#FF4500');
            g.sound.play('invade');
            this.specialCooldown = 250;
